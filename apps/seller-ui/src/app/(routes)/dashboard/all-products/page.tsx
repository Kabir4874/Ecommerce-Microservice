...
const fetchProducts = async () => {
  const res = await api.get("/product/api/get-shop-products");
  return res?.data?.products;
};

const deleteProduct = async (productId: string) => {
  await api.delete(`/product/api/delete-product/${productId}`);
};

const restoreProduct = async (productId: string) => {
  await api.put(`/product/api/restore-product/${productId}`);
};

const ProductList = () => {
  const [globalFilter, setGlobalFilter] = useState("");
  const [anylyticsData, setAnalyticsData] = useState(null);
        ...
        cell: ({ row }: any) => {
          return (
            <Image
              ...
              alt={row.original.images[0]?.url || "profile image"}
              width={48}
              height={48}
